...
import com.informatics.practice.runer.PracticeApplication;
...
public  abstract class AbstractTest extends BaseControllerTest {
    ...
    protected int getRequestTest(String url, PersonDto dto) throws Exception {
        RequestBuilder requestBuilder = MockMvcRequestBuilders.get(url).accept(MediaType.ALL)
                .content(getObjectMapper().writeValueAsBytes(dto)).contentType(MediaType.APPLICATION_JSON_VALUE).header(tokenHeaderName, "Basic " + Base64.getEncoder().encodeToString((username + ":" + password).getBytes()));
    ...
    protected int findAllRequestTest(String url) throws Exception {
        RequestBuilder requestBuilder = MockMvcRequestBuilders.get(url).accept(MediaType.ALL)
                .contentType(MediaType.APPLICATION_JSON_VALUE).header(tokenHeaderName, "Basic " + Base64.getEncoder().encodeToString((username + ":" + password).getBytes()));
        MvcResult result = mockMvc.perform(requestBuilder).andDo(print())
                .andExpect(status().isOk())
                .andReturn();
        int response = result.getResponse().getStatus();
    ...
    protected int deleteRequestTest(String url, Long id) throws Exception {
        RequestBuilder requestBuilder = MockMvcRequestBuilders.delete(url).accept(MediaType.ALL)
                .content(getObjectMapper().writeValueAsBytes(id)).contentType(MediaType.APPLICATION_JSON_VALUE)
                .header(tokenHeaderName, "Basic " + Base64.getEncoder().encodeToString((username + ":" + password).getBytes()));
        MvcResult result = mockMvc.perform(requestBuilder).andReturn();
        int response =result.getResponse().getStatus();
        return response;
